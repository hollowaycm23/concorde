...
// Servidor remoto (Cloudflare Tunnel / IP) - localStorage; vazio = localhost
...
window.showRemoteModal = async () => {
  ...
  if (!clean) { localStorage.removeItem('concorde_remote_url'); alert('Voltando ao servidor local. Recarregue a página.'); }
  else { try { new URL(clean); } catch(e) { return alert('URL inválida'); } localStorage.setItem('concorde_remote_url', clean); alert('Servidor remoto salvo: ' + clean + '\nRecarregue a página para conectar.'); }
  location.reload();
};
(function updateRemoteLabel(){ const upd=()=>{const cur=localStorage.getItem('concorde_remote_url'); const txt=cur?cur:'Local (localhost:3000)'; const el=document.getElementById('remote-label-auth'); if(el) el.textContent=txt;}; if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', upd); else upd(); })();
